...
3. ширина - если нужна вся ширина, то можно получить ширину родителя `document.querySelector(svg_id).parentElement.clientWidth`
4. высота
...
```javascript
qwe.points(arr, false, max, min);
```

#### 6\. Если нужно задать порядок округления чисел, у объекта класса `controlChart` вызвать метод `setDecimalPlaces` (не обязательный) с параметрами:

1. задает до какого знака будут округляться значения в подписи к оси Y (по умолчанию значение 0, не обязательное)
2. задает до какого знака будут округляться расчетные значения (по умолчанию значение 1, не обязательное)

```javascript
qwe.setDecimalPlaces(1, 3); 
```

#### 7\. У объекта класса `controlChart` вызвать метод `calc` с параметрами:
...
3. признак фиксирования масштаба (`false`/`true`). по умолчанию `false`, не обязательный.
Данный параметр влияет на то, как будет отображаться масштаб карты при перерасчете значений (например, если отключить одну из точек). При значении `false` масштаб будет оптимальным с точки зрения отображения информации. При значении `true` карта после перерасчета будет оставаться в том же масштабе.

```javascript
let limits = new limitsClass();  
qwe.calc(limits, false, false); 
// или 
qwe.calc();
```

#### 8\. Если нужно задать подписи к осям, у объекта класса `controlChart` вызвать метод `setAxis` с параметрами:
...
#### 9\. Если нужно задать названия к картам, у объекта класса `controlChart` вызвать метод `setCaptions` с параметрами:

1. Название карты 1 (по умолчанию значение "", не обязательный).
2. Название карты 2 (по умолчанию значение "", не обязательный).

#### 10\. Если нужно задать фиксированный масштаб для карты, у объекта класса `controlChart` вызвать метод `fixScale` с параметрами:

1. Название карты
2. Максимальное значение по y 
3. Минимальное значение по y

#### 11\. У объекта класса `controlChart` вызвать метод `paint` с параметрами:
...
#### 11\. добавить обработчик при наведении на точку `setValueFunc(func)`. не обязательно. по умолчанию показывается значение по оси y. в функцию передаются следующие значение:
...
// можно использовать асинхронные функции
func = async (v, l, i, id) => {
	return await async_chtoto;
}
```

#### 13\. добавить обработчик при наведении на дополнительную линию `setSublineValueFunc(func)`. не обязательно. по умолчанию показывается значение id дополнительной линии. в функцию передаются следующие значение:
...
#### 14\. если надо показать требования потребителя, то надо вызвать метод `setRequirements(bottom, top)`:

```javascript
qwe.setRequirements(bottom, top);
```
